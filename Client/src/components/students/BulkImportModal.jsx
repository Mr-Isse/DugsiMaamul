...
import { useBulkImportStudentsMutation } from '@/services/api'
...
const BulkImportModal = ({ isOpen, onClose }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const result = await bulkImportStudents(formData).unwrap()
      setUploadResult(result)
      toast.success(`Successfully imported ${result.summary?.created || 0} students`)
    } catch (err) {
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 dark:bg-gray-800">
                        <tr>
                          ...
                          <th className="px-4 py-2 text-left text-xs font-semibold text-gray-600 dark:text-gray-400">Parent</th>
                        </tr>
                      </thead>
                      <tbody>
                        {previewData.map((row, i) => (
                          <tr key={i} className="border-t border-gray-100 dark:border-gray-700">
                            ...
                            <td className="px-4 py-2 text-gray-600 dark:text-gray-400">{row.parentName}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {uploadResult.error ? (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-sm text-gray-500 text-center">
                    {uploadResult.message || 'There was an error importing the students'}
                  </p>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-sm text-gray-500 text-center">
                    {uploadResult.importedCount || 'Students'} have been imported successfully
                  </p>
                </div>
              )}
