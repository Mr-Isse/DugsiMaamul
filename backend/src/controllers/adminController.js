...
export const generateCertificate = async (req, res) => {
  ...
  try {
    ...
    const pdfBuffer = await generatePdf('certificate', {
      ...
      verificationNumber: certificate.certificateNumber,
...
export const getAdmissions = async (req, res) => {
  try {
    ...
    const admissions = await Admission.find({ school: schoolId, branch: branchId }).populate('class');
    res.json(admissions);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
export const getAdmission = async (req, res) => {
  try {
    const { id } = req.params;
    const schoolId = req.user.school?._id || req.user.school;
    const admission = await Admission.findOne({ _id: id, school: schoolId }).populate('class');
    if (!admission) {
...
export const createAdmission = async (req, res) => {
  try {
    ...
    // Verify class exists
    const classExists = await Class.findOne({ _id: classId, school: schoolId, branch: branchId });
    if (!classExists) {
...
export const updateAdmission = async (req, res) => {
  try {
    ...
    const admission = await Admission.findOne({ _id: id, school: schoolId, branch: branchId });
    if (!admission) {
      return res.status(404).json({ message: 'Admission not found' });
    }
    ...
    // Update fields
    if (studentName !== undefined) admission.studentName = studentName.trim();
    if (email !== undefined) admission.email = email?.trim().toLowerCase();
    if (phone !== undefined) admission.phone = phone?.trim();
    if (dateOfBirth !== undefined) admission.dateOfBirth = dateOfBirth;
    if (gender !== undefined) admission.gender = gender;
    if (parentName !== undefined) admission.parentName = parentName.trim();
    if (parentPhone !== undefined) admission.parentPhone = parentPhone.trim();
    if (address !== undefined) admission.address = address;
    if (previousSchool !== undefined) admission.previousSchool = previousSchool;
    if (documents !== undefined) admission.documents = documents;
...
export const createExam = async (req, res) => {
  const { name, term, date, classes, subjects, maxMarks, duration, passingScore } = req.body;
  const schoolId = req.user.school?._id || req.user.school;
  try {
    ...
    if (!targetBranchId && classes && classes.length > 0) {
      const cls = await Class.findById(classes[0]);
      targetBranchId = cls?.branch;
    }

    // Handle single or multiple classes and subjects
    ...
    // Create exam for each class-subject combination
    const exams = [];
