...
export const createPayroll = asyncHandler(async (req, res) => {
  ...
  const employee = await User.findOne({
    _id: userId, school: schoolId, isDeleted: { $ne: true },
  ...
  const duplicate = await Payroll.findOne({
    school: schoolId, branch: branchId, user: userId, month: Number(month), year: Number(year), isDeleted: { $ne: true },
  });
  ...
  if (salaryStructureId) {
    structure = await SalaryStructure.findOne({ _id: salaryStructureId, school: schoolId, isDeleted: { $ne: true } });
  }
  ...
  const seq          = await Payroll.countDocuments({ school: schoolId }) + 1;
