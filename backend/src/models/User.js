...
userSchema.virtual('imageUrl').get(function() {
  ...
  const baseUrl = typeof this.profileImage === 'string' ? this.profileImage : this.profileImage.url;
  if (!baseUrl) return undefined;
  const updatedAt = this.imageUpdatedAt || this.updatedAt || Date.now();
  const sep = baseUrl.includes('?') ? '&' : '?';
  return `${baseUrl}${sep}t=${new Date(updatedAt).getTime()}`;
});

userSchema.virtual('imageUpdatedAt').get(function() {
  if (!this.profileImage) return undefined;
  if (typeof this.profileImage === 'object' && this.profileImage.created_at) {
    return new Date(this.profileImage.created_at * 1000);
  }
  if (typeof this.profileImage === 'object' && this.profileImage.updatedAt) {
    return new Date(this.profileImage.updatedAt);
  }
