...
import {
  ShieldAlert, Plus, Search, Edit3, X, AlertTriangle, CheckCircle, TrendingUp, User, Filter,
} from 'lucide-react';
...
const STATUSES = ['Open', 'Mitigating', 'Mitigated', 'Accepted', 'Closed'];
const CATEGORIES = ['Financial', 'Operational', 'Strategic', 'Compliance', 'Safety'];
...
const RiskModal = ({ initial, onClose }) => {
  ...
  const riskScore = Number(form.likelihood) * Number(form.impact);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.title.trim()) return toast.error('Title is required');
    try {
      const payload = { ...form, likelihood: Number(form.likelihood), impact: Number(form.impact), riskScore };
      if (isEdit) {
        await updateRisk({ id: initial._id, ...payload }).unwrap();
      ...
      toast.error(err?.data?.message || 'Failed to save risk');
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Likelihood (1-5)</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Impact (1-5)</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Score</label>
              <div className={`w-full px-4 py-2.5 rounded-xl text-sm font-black text-center ${getScoreColor(riskScore)}`}>
                {riskScore}
              </div>
            </div>
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Mitigation Plan</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5">Risk Owner</label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={creating || updating}
              ...
              {creating || updating ? 'Saving...' : isEdit ? 'Update Risk' : 'Create Risk'}
            </button>
          </div>
        </form>
      </div>
...
const DeleteModal = ({ risk, onClose }) => {
  const [deleteRisk, { isLoading }] = useDeleteRiskMutation();

  const handleDelete = async () => {
    try {
      await deleteRisk(risk._id).unwrap();
      toast.success('Risk deleted');
      onClose();
    } catch (err) {
      toast.error(err?.data?.message || 'Failed to delete risk');
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 w-full max-w-sm p-6">
        ...
        <h2 className="text-xl font-black text-center text-slate-900 dark:text-white mb-2">Delete Risk</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 text-center mb-6">
          Are you sure you want to delete "<span className="font-bold text-slate-700 dark:text-slate-200">{risk.title}</span>"?
        </p>
...
const RiskRegister = () => {
  ...
  const { data, isLoading, refetch } = useGetRiskRegistersQuery(queryArgs);
  const risks = data?.data || data?.risks || (Array.isArray(data) ? data : []);

  const stats = useMemo(() => ({
    total: risks.length,
    highPriority: risks.filter(r => (r.riskScore || r.likelihood * r.impact) > 12).length,
    mitigated: risks.filter(r => r.status === 'Mitigated').length,
    underReview: risks.filter(r => r.status === 'Open' || r.status === 'Mitigating').length,
  }), [risks]);

  const statCards = [
    { label: 'Total Risks', value: stats.total, icon: ShieldAlert, color: 'bg-indigo-500' },
    { label: 'High Priority', value: stats.highPriority, icon: AlertTriangle, color: 'bg-red-500' },
    { label: 'Mitigated', value: stats.mitigated, icon: CheckCircle, color: 'bg-green-500' },
    { label: 'Under Review', value: stats.underReview, icon: TrendingUp, color: 'bg-yellow-500' },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-700 dark:text-slate-300">No risks found</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              {search || statusFilter ? 'Try adjusting your filters.' : 'Click "New Risk" to add one.'}
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1000px] text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {risks.map(risk => {
                  const score = risk.riskScore || (risk.likelihood || 0) * (risk.impact || 0);
                  return (
                    <tr key={risk._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-6 py-4 text-sm text-slate-900 dark:text-white whitespace-nowrap">
                        <div className="font-bold">{risk.title}</div>
                        {risk.description && <div className="text-xs text-slate-400 truncate max-w-[200px]">{risk.description}</div>}
                      </td>
                      <td className="px-6 py-4 text-sm text-slate-900 dark:text-white whitespace-nowrap">{risk.category || '—'}</td>
                      <td className="px-6 py-4 text-sm text-slate-900 dark:text-white whitespace-nowrap text-center font-bold">{risk.likelihood || '—'}</td>
                      <td className="px-6 py-4 text-sm text-slate-900 dark:text-white whitespace-nowrap text-center font-bold">{risk.impact || '—'}</td>
                      ...
                      <td className="px-6 py-4 text-sm whitespace-nowrap">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${STATUS_COLORS[risk.status] || ''}`}>
                          {risk.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setEditRecord(risk)} title="Edit"
                          ...
                          <button onClick={() => setDeleteRecord(risk)} title="Delete"
      ...
      {showCreate && <RiskModal onClose={() => setShowCreate(false)} />}
      {editRecord && <RiskModal initial={editRecord} onClose={() => setEditRecord(null)} />}
      {deleteRecord && <DeleteModal risk={deleteRecord} onClose={() => setDeleteRecord(null)} />}
    </div>
  );
};
